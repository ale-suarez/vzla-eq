...
export const engineerApplicationSubmitSchema = z
  .object({
    ...
    latitude: z.number().min(-90).max(90).openapi({ example: 10.4806 }),
    longitude: z.number().min(-180).max(180).openapi({ example: -66.9036 }),
    address: z.string().optional().openapi({ example: "Av. Bolívar, Caracas, Venezuela" }),
